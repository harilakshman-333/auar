...
const StackSchema = zod_1.z.object({
    id: zod_1.z.string().describe('Unique stack ID, e.g. STACK-01'),
    delivery_day: zod_1.z.number().describe('Delivery day number'),
    total_weight_kg: zod_1.z
        .number()
    ...
    panelIds: zod_1.z
        ...
        .describe('Array of Panel IDs in STRICT LIFO order — first element is the FIRST panel the framer needs (top of stack)'),
});
const CommandResponseSchema = zod_1.z.object({
    type: zod_1.z
...
router.post('/ai-plan', async (req, res) => {
    try {
        ...
        const currentPanels = (0, db_js_1.getAllPanels)();
        const activePanels = currentPanels.filter((p) => p.status === 'pending' || p.status === 'stacked');
        const project = (0, db_js_1.getProjectInfo)();
...
- You should assign all remaining active panels to appropriate delivery days and group them into stacks based on the constraints. Do not leave panels unassigned unless the PM's command explicitly requests to exclude them or to only plan a specific day. All planned panels should be output in the stacks.`;
        ...
        if (result.object.type === 'plan_update' && result.object.stacks) {
            (0, db_js_1.setStacks)(result.object.stacks);
        }
        res.json({
            stacks: (0, db_js_1.getStacks)(),
...
router.get('/', (req, res) => {
    ...
    if (dayParam) {
        ...
        const delivery = (0, db_js_1.getDeliveryByDay)(day);
        if (!delivery) {
    ...
    res.json((0, db_js_1.getAllDeliveries)());
});
